...
Various notions of clusterability has been proposed in the past. The majority of work focus on the fact that clustering can be done efficiently on data sets that follow some nice structure i.e. they are well clusterable.

Bilu et al \cite{bilu2012stable} consider a data set to be nice if small multiplicative perturbations of the input does not change the optimal clustering of it.


Another line of work, focus on center-based clustering tasks. It is assumed that the input data set has a clusterability assumption and an efficient clustering algorithms is designed to output a clustering with near-optimal cost. Ackerman et. al \cite{ackerman2009clusterability} proposed a notion of clusterability based on the fact that the cost of optimal clustering of a data set should not change considerably if the centers of optimal clusters are slightly perturbed.  
...
\vspace{-10mm}
\noindent {\bf Goal}: Given a $(\lambda, \nu, \eta)$-nice set $\mc X$ and $k$, our goal is to design an algorithm that outputs a $k$-clustering $\mc C' =\{C_1',\ldots,C_k'\}$ of $\mc X$ with the following properties. Let $\mc B = \{B_1,\ldots,B_k\}$ be some set which is $\lambda$-separated, $\nu$-dense and $\eta$-sparse. Then the clustering $\mc C'$ should be consistent with $\mc B$. That is, 
\begin{align*}
...
Note that given any set $\mc X$ which is $(\lambda, \nu, \eta)$-nice, there can exist different sets $\mc B$ which satisfy the separation, dense and sparseness conditions. We require that a clustering $\mc C'$ outputed by an algorithm should be consistent with one such $\mc B$. The following question arises. 

{\em Is it possible to output a clustering $\mc C'$ which is consistent with all such $\mc B$'s?}

If such a $\mc C'$ exists then we say that our solution is `unique' (for all $\mc B$'s). In this section, we wish to examine conditions under which the solution is unique. 

Given any set $\mc B = \{B_1,\ldots, B_k\}$ which is $\lambda$-separated, $\nu$-dense and $\eta$-sparse. We parametrize it by the following two quantities. $r = \max_i r(B_i)$ and $t = \min_i |B_i|$.
...
Thm. \ref{theorem:uniqueR} shows that for a fixed value of $r$, $\mc C(r)$ is the unique solution for all $\mc B'$ such that $\max_i r(B_i') = r$. In Section \ref{section:fixedR}, we will present an algorithm that finds this solution. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Algorithm}
\subsection{Based on the knowledge of $\max r(B_i)$}
\label{section:fixedR}
