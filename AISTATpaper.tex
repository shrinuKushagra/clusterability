...
Various notions of clusterability has been proposed in the past. The majority of works focus on the fact that clustering can be done efficiently on data sets that follow some nice structure i.e. they are well clusterable.
\cite{ackerman2009clusterability} proposed a notion of clusterability based on the fact that the cost of optimal clustering of a data set should not have considerable change if the centres of optimal clusters are slightly perturbed. 
%Using this assumption, an efficient clustering algorithm is presented that outputs a clustering with near-optimal cost. 
Similarly, \cite{bilu2012stable} consider a data set to be nice if small multiplicative perturbations of the input does not change the optimal clustering of it.
