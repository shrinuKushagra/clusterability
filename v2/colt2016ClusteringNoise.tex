...
\subsection{Outline}
The rest of this paper is structured as follows. In Section \ref{sec:Notation}, we present notation and formal definitions which are necessary to explain our results. in Section \ref{section:cp}, we consider a set $\mc X$ that contains a clustering which satisfies $(\alpha,\eta)$-center proximity and propose an efficient algorithm which recovers all the underlying clusterings of $\mc X$ which satisfies $(\alpha,\eta)$-center proximity; in the presence of structureless noise. Furthermore, we prove that any relaxation of our assumption on noise, or values of $(\alpha,\eta)$ will not be possible.
Section \ref{sec:cs} has a similar structure with focus on the input sets with underlying $\lambda$-center separated clusterings

% we present our main positive and negative result for recovering all $\alpha$-center proximal clusterings of a set which contains either arbitrary or structureless noise. 
