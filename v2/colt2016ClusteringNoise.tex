...
In the past few years, there has been a line of work on defining notions of clusterability. The goal of all these methods has been to show that clustering will be computationally efficient if the input $\mc X$ enjoy some nice structure. \cite{bilu2012stable} consider a clustering instance to be \emph{stable} if the optimal solution to a given objective function does not change under small multiplicative perturbations of distances between the points. Using this assumptoion, they give an efficient algorithm to find the max-cut clustering of graphs which are resilient to $O(\sqrt{|\mc X|})$ perturbations. USing a similar assumption, \cite{ackerman2009clusterability} consider additive perturbations of the underlying metric and designed an efficient algorithm that outputs a clustering with near-optimal cost. 


%\cite{awasthi2012center}. They define a data set $\mathcal{X}$ to have $\alpha$-center proximity w.r.t. an objective function $\Phi$, if the optimal clustering of $\mc{X}$ wr.t. $\Phi$ satisfies .... .
Later on, \cite{awasthi2012center} studied {\em $\alpha$-center proximity} condition (look at Def.~\ref{defn:alphacp}), as a special case of \cite{bilu2012stable} condition.
Assuming that the centers belong to $\mc X$ ({\em proper} setting), they propose an efficient algorithm that outputs the optimal solution of a given center-based objective on $(\alpha > 3)$-center proximal inputs. This result was improved to $(\alpha = \sqrt{2} + 1 \approx 2.4)$ when the objective is $k$-median \cite{balcan2012clustering}.
In the case that centers are arbitrary point of the metric space ({\it steiner} setting), an efficient algorithm is proposed for $(\alpha > 2+\sqrt{3}\approx 3.7)$-center proximal inputs \cite{awasthi2012center}, furthermore, it has been proved that it is NP-Hard to get any efficient algorithm for $\alpha\leq 3$. 
%They proved that finding the optimal solution of any center-based objective to is NP-Hard if the input is  (in the steiner setting).
% \cite{balcan2012clustering} showed that it is possible to overcome the $(\alpha = 3)$ NP-Hardness lower bound in the proper setting. They gave a polynomial-time algorithm that finds the optimal solution of $k$-median objective function for $(\alpha = \sqrt{2} + 1 \approx 2.4)$-center proximal inputs. 

The common component of all these studies is that they assume that all the points of the domain enjoy some {\em nice} property. This requirement can be strict in many real-world situations. A more realistic setting would be the one in which the input dataset has some nice structure. However, this structure is ``disturbed" by the addition of unstructured points ({\it noise}). Detecting the structure of underlying clustering can get difficult in the presence of adversarial noise.

Few methods have been suggested for analyzing clusterability in the presence of noise. 
\cite{balcan2012clustering} consider a dataset which has $\alpha$-center proximity except for an $\epsilon$ fraction of the points. They give an efficient algorithm which provides a $1+O(\epsilon)$-approximation to the cost of the $k$-median optimal solution when $\alpha > 2+\sqrt{7} \approx 4.6$. The main limitation of such a method is that the approximation guarantee is heavily influenced by the size of noise; which makes it hard to justify using it for a big class of real-world data sets.
%\cite{balcan2012clustering} presented an extension of their result when $\Phi$ is $k$-median objective function and the clustering instance contains $\epsilon$ fraction of it as noise. In this setting, they construct a hierarchical clustering tree of $\mc{X}$ which captures the optimal clustering $\mc{C}^*$ on the non-noisy points. Their result is restricted to the case that size of noise is bounded by $t(\mc{C}^*)/8$.

In a different line of work, \cite{ben2014clustering} studies the problem of robustifying any center-based clustering objective to noise. To achieve this goal, they introduce the notion of {\em center separation} (look at Def. \ref{defn:lambdacs}). Informally, an input has center separation when it can be covered by $k$ well-separated set of balls.
...
%They work with a more relaxed definition where the input dataset satisfies center separation except for a small fraction of the points. They then introduce a generic algorithm for how to modify any center-based algorithm to output the underlying $k$-clustering of the {\em un-noisy} part of the data.
%\cite{ben2014clustering} introduced {\em center separation}. Their goal is to robustify any objective based clustering algorithm to noise.  In this direction, they define a data set $\mc X$ to be clusterable, if the optimal solution of any objective based clustering algorithm on $\mc X$ has $\lambda$-center sepration. Given such an input, they propose a paradigm which converts any objective-based clustering algorithm into a clustering algorithm which is robust to small amount of noise. 
...
% if $\mc X$ has a clustering with $(\lambda > 10)$-center separation. 

In this work, we consider a natural relaxation of \cite{balcan2012clustering, ben2014clustering}, with the goal to capture more realistic domains containing arbitrary amount of noise. 
%One of the main contributions of this paper is 
We define a novel quantitative measure of noise that is close to reality. Informally, we call noise {\em unstructured}if it is scattered sparsely among the data and does not have similar structure to a {\em nice} cluster at any part of the domain. This definition (look at Def.~\ref{def:alphaeta}), does not force any restriction on the size of the noise but rather on the structure of it.

Another important aspect of our work is to design clustering algorithms that counts for variability of underlying {\em nice} clusterings of the data. We do not want to output a single clustering but to produce an efficient representation of all possible {\em nice} clusterings. This will allow the user to be able to chose his target clustering according to his objective.

Given a clusterable input $\mc X$ which contains {\em unstructured} noise, we propose an efficient algorithm that outputs a hierarchical clustering tree of $\mc X$ that captures all {\em nice} clusterings of $\mc X$. Our algorithm perfectly recovers the underlying {\em nice} clusterings of the input and its performance is independent of number of noisy points in the domain. 

Finally, we study the limitations of any efficient clustering algorithm in the presence of arbitrary amount of noise. These results show that, our {\em unstructured} noise assumption actually empowers the clustering algorithms to overcome arbitrary amount of noise in larger class of data sets.
...
\noindent The set $\mc X$ is {\it $\lambda$-separated} if there exist a clustering $\mc C_{\mc X}$ which satisfies $\lambda$-center separation.
...
\noindent A set $\mc S \subseteq \mc X$ is {\it $(\lambda, \eta)$-separated} if there exist a clustering $\mc C_{\mc S}$ which satisfies $(\lambda, \eta)$-center separation. Note that given $(\mc X, d)$ and $k$ there can be several such $\mc S$ and several clusterings $\mc C_{\mc S}$ which satisfy these conditions.
...
Any clustering The set $\mc X$ is {\it $\alpha$-center proximal} if there exist a $k$-clustering $\mc C_{\mc X}$  which satisfies $\alpha$-center proximity. 

A set $\mc S \subseteq \mc X$ is {\it $(\alpha, \eta)$-proximal} if there exist a $k$-clustering $\mc C_{\mc S}$ which satisfies $(\alpha, \eta)$-center proximity. Note that given $(\mc X, d), k$ there can be several such $\mc S$ and several clusterings $\mc C_{\mc S}$ which satisfy these conditions.


Here are the main results of this section:

\begin{itemize}
\item {\it Lower bound with arbitrary noise} - In Section \ref{section:alphaLowerBoundArbitrary}, we show that if the number of noisy points exceeds $1.5$(size of the smallest cluster) and $\alpha \le 2\sqrt{2} + 3 \approx 5.8$, then there is no tree which captures all $\alpha$-center $t$-min proximal subsets of $\mc X$.\samira
\item  {\it Positive result under sparse noise} - In Section \ref{section:positiveResultSparseNoise}, we show that we can overcome this lower bound if we require that the noise is sparse. If $\alpha \ge 2 + \sqrt{7} \approx 4.6$ and $\eta \ge 1$, then Alg.\ref{alg:alphacp} outputs a tree which captures all $(\alpha, \eta)$-proximal nice subsets of $\mc X$. Our algorithm runs in time polynomial in $\mc X$. 
...
Given the number of clusters $k$ and a parameter $t$. For all $\alpha < 2\sqrt 2 + 3$ there exists a clustering instance $(\mc X, d)$ such that any clustering tree $\mc T$ of $\mc X$ has  the following property. There exists $\mc S \subseteq \mc X$ and there exists clustering $\mc C_{\mc S} = \{S_1, \ldots, S_k\}$ which satisfies $\alpha$-center proximity w.r.t $\mc S$ and $k$ such that the following holds. 
...
\begin{theorem}
Given a clustering instance $(X , d)$. For all $\lambda \ge 3$, Alg. \ref{alg:lambdacsNoNoise} outputs a tree $\mc T$ with the following property. For all $k$ and for all $k$-clusterings $\mc C_{\mc X}^* = \{C_1^*, \ldots, C_k^* \}$ of the set $\mc X$ which satisfy $\lambda$-center separation w.r.t $\mc X$ and $k$, the following holds.
