...
In the past few years, there has been a line of work on defining notions of clusterability. The goal of all these methods has been to show that clustering will be computationally efficient for inputs that enjoy some nice structure. \cite{bilu2012stable} consider a clustering instance to be \emph{stable} if the optimal solution to the objective function does not change under small multiplicative perturbations of distances between the points. Specially, they give an efficient algorithm to find the max-cut clustering of graphs which are resilient to $\alpha = O(\sqrt{n})$ perturbations. 

Later on, \cite{awasthi2012center}, considered a similar condition ($\alpha$-center proximity). They propose an efficient algorithm that outputs the optimal solution of a given objective-based clustering task on $(\alpha > 3)$-center proximal inputs when the centers are part of the data set ({\it proper} setting), and on $(\alpha > 2+\sqrt{3})$-center proximal inputs when the centers are arbitrary points of the metric space ({\it steiner} setting). They also prove that finding the optimal solution of any objective-based clustering task is NP-Hard if the input is $(\alpha \leq 3)$-center proximal (in the steiner setting). Following this result, \cite{balcan2012clustering} showed that it is possible to overcome the $(\alpha = 3)$ NP-Hardness lower bound in the proper setting. They gave a polynomial-time algorithm that finds the optimal solution of $k$-median objective function for $(\alpha = \sqrt{2} + 1 \approx 2.4)$-center proximal inputs. 


\samira {\textcolor{blue}{[Edited the related work up to here}\xspace]}


A different line of enquiry was considered by \cite{ackerman2009clusterability}. Rather than multiplicative perturbations, they consider an input such that the cost of the optimal clustering does not change under small additive perturbations of the underlying metric. They designed an efficient algorithm that outputs a clustering with near-optimal cost. 

The notions of multiplicative and additive resilience have one thing in common. They assume that all the points in the given input enjoy the {\em nice} property. This requirement can be strict in many situations. A more realistic setting would be the one in which the input dataset has some nice structure. However, this structure is `disturbed' by the addition of unstructured points (or {\it noise}) to the dataset. The {\it noise} makes it harder to detect the structure of the remaining data points. 

.\cite{balcan2012clustering} consider a dataset which has $\alpha$-center proximity except for an $\epsilon$ fraction of the points. They give an efficient algorithm which provides a $O(1+\epsilon/\rho)$-approximation to the cost of the $k$-median optimal solution when $\alpha > 2+\sqrt{7}$. 

In a different line of work, \cite{ben2014clustering} introduced a notion of $\lambda$-center separation (Defn. \ref{defn:lambdacs}). Informally, an input dataset has center separation when it can be covered by $k$ well-separated set of balls. They work with a more relaxed definition where the input dataset satisfies center separation except for a small fraction of the points. They then introduce a generic algorithm for how to modify any center-based algorithm to output the underlying $k$-clustering of the {\em un-noisy} part of the data.
