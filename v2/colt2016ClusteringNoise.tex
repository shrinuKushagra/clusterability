...
In the case that centers are arbitrary point of the metric space ({\it steiner} setting), an efficient algorithm is proposed for $(\alpha > 2+\sqrt{3}\approx 3.7)$-center proximal inputs \cite{bilu2012stable} -- this result is tight in the steiner setting. 
...
In a different line of work, \cite{ben2014clustering} studies the problem of robustifying any center-based clustering objective to noise. To achieve this goal, they introduce the notion of center separation (look at Def. \ref{defn:lambdacs}). Informally, an input has center separation when it can be covered by $k$ well-separated set of balls.
...
In this work, we consider a natural relaxation of \cite{balcan2012clustering, ben2014clustering}, with the goal to capture more realistic domains containing significant amount of noise.  work is We study the limitations of any efficient clustering algorithm in the presence of arbitrary amount of noise. Furthermore, assuming that noise is {\em sparse}, we propose an efficient algorithm that outputs a hierarchical clustering tree of $\mc X$ that captures all clusterings of all $(\alpha, \eta)$-proximal nice subsets of $\mc X$.
