...
.\cite{awasthi2012center}, consider a similar condition ($\alpha$-center proximity) on the input of center-based clustering tasks. They propose an efficient algorithm that outputs the optimal clustering for $\alpha > 3$ when the centers are part of the data-set ({\it proper} setting) and for $\alpha > 2+\sqrt{3}$ when the centers are arbitrary points of the metric space ({\it steiner} setting). They also prove NP-Hardness results for $\alpha = 3$ in the steiner setting. \cite{balcan2012clustering} showed that it is possible to overcome the $\alpha = 3$ NP-Hardness lower bound in the proper setting. They gave a polynomial-time algorithm for the $k$-median objective function for $\alpha = \sqrt{2} + 1 \approx 2.4$. 
