...
Given a hierarchical clustering tree $\mc T$ of $\mc X$, we say that a pruning $\mc P$ respects $\mc C_{\mc S}$ if the clustering $\mc C_{\mc X}^{\mc P}$ corresponding to the pruning respects $\mc C_{\mc S}$. Similarly, we say that $\mc T$ respects $\mc C_{\mc S}$ if there exists a pruning $\mc P$ which respects $\mc C_{\mc S}$.
\end{definition}
...
Note: In the setting of this paper, we will restrict the centers $c_i \in \mc X$. 
\end{definition}
...
If $\lambda < 4$ and $\eta \le 1$, then there doesn't exist a pruning of the tree that respects $\mc C_{\mc S}$.
\end{theorem}
