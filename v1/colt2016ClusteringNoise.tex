...
\label{defn:lambdacs}
Given a clustering instance $\mc X$ drawn from some metric space $(\mb M, d)$ and the number of clusters $k$. We say that a clustering $\mc C_{\mc X} = \{C_1, \ldots, C_k\}$ induced by centers $c_1, \ldots, c_k \in \mb M$ has $\lambda$-center separation w.r.t $\mc X$ and $k$ if the following holds. For all $i\neq j$, 
...
Given a clustering instance $\mc X$ drawn from a metric space $(\mb M, d)$, the number of clusters $k$ and $\mc S \subseteq \mc X$. We say that a clustering $\mc C_{\mc S} = \{S_1, \ldots, S_k\}$ induced by centers $s_1, \ldots, s_k \in \mb M$ has $(\lambda, \eta)$-center separation w.r.t $\mc X, \mc S$ and $k$ if the following holds.
...
We will prove that Alg. 1 in \cite{balcan2012clustering} achieves the above mentioned goal when $\lambda \ge 3$. The algorithm uses a linkage procedure which is based on closure distance (Defn. 6 in \cite{balcan2012clustering}).
\begin{definition}[Closure distance]
Given two sets $S$ and $S'$, the closure distance $d_c(S, S')$ between them is defined as the minimum $d$ such that there exists $c \in S \cup S'$ with the following properties.
\begin{itemize}[noitemsep, leftmargin=*]
\item $S \cup S' \subseteq B(c, d)$.
\item For all $p \in B(c, d)$ and for all $q \not\in B(c, d)$, $d(p, c) < d(p, q)$ .
\end{itemize}
\end{definition}

Intuitively, the algorithm  works as follows. It maintains a clustering $\mc C_{\mc X}$, which is initialized so that each point is in its own cluster. It then merges clusters which minimize the closure distance. For completeness, we give a short description of the algorithm (Alg. \ref{alg:lambdacsNoNoise}). 
\begin{algorithm}[t]
%\begin{alg}
	\SetAlgoLined
	\KwIn{$(\mc X, d)$ and $k$}
	...
	\vspace{2mm}Let $\mc C^{(l)}$ denote the clustering $\mc X$ after $l$ merge steps have been performed. Initialize $\mc C^{(0)}$ so that all points are in their own cluster. That is, $\mc C^{(0)} = \{ \{x\}: x \in \mc X\}$.
	
	\vspace{2mm} Let $\mc C^{(l)} = \{C_1, \ldots, C_m\}$. Merge clusters $C$ and $C'$ $$C, C' = \argmin_{C_i, C_j \in C^{(l)}} \thinspace d_c(C_i, C_j).$$
...
\begin{proof}
\todo The idea of the proof of this theorem is identical to the proof of Thm. 1 in \cite{balcan2012clustering}. For completeness, we
\end{proof}
